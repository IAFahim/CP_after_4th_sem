...
public class NumberofApartments {
    ...
    public static void main(String[] args) throws IOException {
        int till = sc.nextInt();
        boolean present[] = new boolean[1010];
        present[0] = true;
        for (int i = 0; i < 1000; i++) {
            if (!present[i]) continue;
            present[i + 3] = present[i + 5] = present[i + 7] = true;
        }
//        for (int i = 0; i < present.length; i++) {
//            out.println(i+" "+present[i]);
//        }
        for (int z = 0; z < till; z++) {
            int x = sc.nextInt();
            if (!present[x]) System.out.println(-1);
            else {
                int count[] = new int[3];
                while (x != 0) {
                    if (present[x - 3]) {
                ...
                out.println(count[0] + " " + count[1] + " " + count[2]);
            }
        }
    ...
    // 5:18 AM Fri 06 2021 25/06/2021
}