...
public class FlippingGame {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < n; i++) {
            for (int j = n - 1; j > i; j--) {
                int first = getSum(0, i);
                int middle = oneFound(i, j, arr);
                int last = getSum(j, n);
                max = max(first + middle + last, max);
            }
        }
        out.print(((max == pre[n]) ? n - 1 : max) + "");

        close.close();
    }

    static int oneFound(int i, int j, int[] arr) {
        int one = 0;
        for (; i <= j; i++) {
            if (arr[i] == 0) {
                one++;
    ...
    // 6:17 PM Sat 07 2021 03/07/2021
}